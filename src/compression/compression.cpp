...
void compress_(ExpansionBlock *m, int &m_size, HashBlock &block)
{
    ...
    for (int i = 0; i < m_size / EXPANSION_BLOCK_COUNT_IN_SET; ++i)
    {
        ExpansionBlock mi = m[i];
        a = hs[0].to_ulong();
        b = hs[1].to_ulong();
        c = hs[2].to_ulong();
        d = hs[3].to_ulong();
        ...
        {
            ExpansionBlock mij = m[i * EXPANSION_BLOCK_COUNT_IN_SET + j];
            Prime t1 = big_sigma0(a).to_ulong() + maj(a, b, c).to_ulong() + big_sigma2(c + d).to_ulong();
            Prime t2 = h + big_sigma1(e).to_ulong() + ch(e, f, g).to_ulong() + ks[j] + mij.to_ulong();
